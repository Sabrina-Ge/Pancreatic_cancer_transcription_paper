...
#' 
singler_subtype <- function(mat, reference_file="bulk_reference.RData", find_mixed=TRUE, ...) {
  ...
  if ( !("Classic1" %in% ref_labels) | !("Basal2" %in% ref_labels) | "hybrid" %in% ref_labels) {
  ...
  if (find_mixed) {
    mixed_cutoff <- 0.02
    # Cells identified as Mixed if originally Classic1 or Basal2, and similar in score for both.
    subtype_with_mixed <- ifelse(data$subtype %in% c("Classic1", "Basal2") & 
